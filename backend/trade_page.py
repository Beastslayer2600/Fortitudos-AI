...
def render_trade_html(facts: TradeFacts, *, include_sku: bool = False, live: bool = False) -> str:
    ...
    # The studio's own name, not this file's idea of it.
    from identity import load
    studio = load().studio_name
    sku = (f"<p>{_esc(studio)}: R{PRICE:,} once, R{DEPOSIT:,} to start.</p>"
           if include_sku and not live else "")
