"""HTTP extras the stock app.py was missing: Learn, Sight, Craft pages, consent."""
...
def handle_get(handler, parts) -> bool:
    ...
    if parts == ["api", "learn", "discover"]:
        ...
        gaps = []
        for item in catalog:
            have = any(item["id"].replace("-", "") in n.replace("-", "").replace("/", "") or item["title"].split()[0].lower() in n.lower() for n in docs)
            gaps.append({"id": item["id"], "title": item["title"], "branch": item["branch"], "have": have})
        handler.send_json({"catalog": catalog, "gaps": gaps, "rule": "File lessons per room. Do not use all unless the rule is shared."})
        return True
    if len(parts) == 2 and parts[0] == "m":
        slug = _slug(parts[1])
        path = MOCK_DIR / f"{slug}.html"
        if not path.exists():
            handler.send_json({"error": "No mock for that slug. Run /api/craft/page first."}, 404)
            return True
...
def handle_post(handler, parts, body) -> bool:
    ...
    if parts == ["api", "ingest", "paste"]:
        from learn_teach import file_lesson
        title = str(body.get("title") or "Paste").strip()
        text = str(body.get("text") or "").strip()
        path = file_lesson(title, text, "all")
    ...
    if parts == ["api", "craft", "page"]:
        ...
        public = str(body.get("url") or "").strip()
        MOCK_DIR.mkdir(parents=True, exist_ok=True)
        slug = _slug(name)
        mock_url = public or f"http://127.0.0.1:8000/m/{slug}"
        ...
        handler.send_json({
            ...
            "slug": slug,
            "path": f"/m/{slug}",
            "flyer": f"/m/{slug}-flyer" if False else str(MOCK_DIR / f"{slug}-flyer.html"),
            "spec": out["spec"],
            "missing": out["missing"],
            "note": "Host /m/{slug} on a public URL before you print the QR.",
        })
        return True
